/*
Assuming the protocol we are adding is for the (imaginary) manufacturer:  TagShot
...
2. Replace all occurrences of "TagShot" with the name of your protocol.
...
1. Open IRremote.h and make the following changes:
   REMEMEBER to change occurences of "TAGSHOT" with the name of your protocol
...
3. Open MyDocuments\Arduino\libraries\IRremote\examples\IRrecvDumpV2.ino
   ...
   A. In the encoding() function, add:
      case TAGSHOT:    Serial.print("TAGSHOT");     break ;
...
//
#if SEND_TAGSHOT
void  IRsend::sendTagShot (unsigned long data,  int nbits)
{
