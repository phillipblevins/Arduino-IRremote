/*
Assuming the protocol we are adding is for the (imaginary) manufacturer:  MilesTag2Shot
...
1. Copy this file to : ir_MilesTag2Shot.cpp

2. Replace all occurrences of "MilesTag2Shot" with the name of your protocol.
...
1. Open IRremote.h and make the following changes:
   REMEMEBER to change occurences of "MILESTAG2SHOT" with the name of your protocol
...
3. Open MyDocuments\Arduino\libraries\IRremote\examples\IRrecvDumpV2.ino
   ...
   A. In the encoding() function, add:
      case MILESTAG2SHOT:    Serial.print("MILESTAG2SHOT");     break ;
...
#define HDR_SPACE   600  // The lenght of the Header:Space
...
//
#if SEND_MILESTAG2SHOT
void  IRsend::sendMilesTag2Shot (unsigned long data,  int nbits)
{
...
//
#if DECODE_MILESTAG2SHOT
bool  IRrecv::decodeMilesTag2Shot (decode_results *results)
{
	...
	results->bits        = BITS;
	results->value       = data;
	results->decode_type = MILESTAG2SHOT;
	return true;
}
#endif
